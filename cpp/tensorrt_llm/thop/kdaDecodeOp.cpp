    ...
    double scale, double onorm_eps)
{
    ...
    tensorrt_llm::kernels::kdaDecode::KdaDecodeParams const params{x_q.data_ptr(), x_k.data_ptr(), x_v.data_ptr(),
        ...
        state.stride(0), out.data_ptr(), B, H, HV, apply_onorm, update_conv_cache, use_lower_bound, apply_beta_sigmoid,
        static_cast<float>(lower_bound), static_cast<float>(scale), static_cast<float>(onorm_eps)};
    ...
    double onorm_eps, std::optional<at::Tensor> output)
{
    ...
    launch_selected_kernel(x_q, x_k, x_v, w_q_t, w_k_t, w_v_t, bias_q, bias_k, bias_v, cs_q, cs_k, cs_v, a_log, g,
        dt_bias, beta, onorm_g, onorm_weight, ssm_state_indices, state, out, apply_onorm, update_conv_cache,
